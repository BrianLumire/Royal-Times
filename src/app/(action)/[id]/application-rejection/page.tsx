...
const ApplicationRejectionPage = ({ params }: ApplicationRejectionPageProps) => {
  const { id } = params;
    ...
    <div className="h-screen bg-gray-100 flex items-center justify-center">
      <div className="w-[95%] sm:w-[60%] lg:w-[40%] bg-white flex flex-col p-3 rounded-xl shadow-md border border-gray-200">
        ...
        <button
          ...
          className={`p-2 mt-3 rounded-[12px] flex border ${selectedOption === "Wrong document" ? activeBorder : "border-gray-300"}`}
        >
          <span className={`font-sans text-sm font-medium ${selectedOption === "Wrong document" ? activeText : ""}`}>
            Wrong document
          </span>
        </button>
        <button
          onClick={() => setSelectedOption("Vehicle propulsion")}
          className={`p-2 mt-3 rounded-[12px] flex border ${selectedOption === "Vehicle propulsion" ? activeBorder : "border-gray-300"}`}
        >
          <span className={`font-sans text-sm font-medium ${selectedOption === "Vehicle propulsion" ? activeText : ""}`}>
            Vehicle propulsion
          </span>
        </button>
        <button
          onClick={() => setSelectedOption("License Plate")}
          className={`p-2 mt-3 rounded-[12px] flex border ${selectedOption === "License Plate" ? activeBorder : "border-gray-300"}`}
        >
          <span className={`font-sans text-sm font-medium ${selectedOption === "License Plate" ? activeText : ""}`}>
            License Plate
          </span>
        </button>
        <button
          onClick={() => setSelectedOption("Brand")}
          className={`p-2 mt-3 rounded-[12px] flex border ${selectedOption === "Brand" ? activeBorder : "border-gray-300"}`}
        >
          <span className={`font-sans text-sm font-medium ${selectedOption === "Brand" ? activeText : ""}`}>
            Brand
          </span>
        </button>
        {/* Footer Buttons */}
