...
import { storeVerificationInfo } from "@/app/actions";
import { toast } from "react-toastify";

// Define the schema for validation
const schema = z.object({
  driverLicenseFront: z
    .any()
    .refine((file) => file, { message: "Driver License Front is required" }),
  driverLicenseBack: z
    .any()
    .refine((file) => file, { message: "Driver License Back is required" }),
  insuranceFile: z
    .any()
    .refine((file) => file, { message: "Proof of Insurance is required" }),
  licensePlateFile: z
    .any()
    .refine((file) => file, { message: "License Plate is required" }),
  proofOfOwnershipFile: z
    .any()
    .refine((file) => file, { message: "Proof of Ownership is required" }),
});
...
const AddDriverVerificationInfoPage = () => {
  ...
  const [licensePlateImage, setLicensePlateImage] = useState<string | null>(
    null
  );
  const [proofOfOwnershipImage, setProofOfOwnershipImage] = useState<
    string | null
  >(null);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    driverData.append("driver_license_front", data.driverLicenseFront);
    driverData.append("driver_license_back", data.driverLicenseBack);
    driverData.append("proof_of_insurance", data.insuranceFile);
    driverData.append("license_plate", data.licensePlateFile);
    driverData.append("proof_of_ownership", data.proofOfOwnershipFile);
    driverData.append("user_id", user_id);

    const storeVerificationInfoResponse = await storeVerificationInfo(
      driverData
    );

    if (storeVerificationInfoResponse.success) {
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-8 ">
              ...
              <div className="flex flex-col w-full md:w-1/2 h-[350px] ">
                <p className="text-base mb-3 font-sans font-medium">
                  Driver License *
                </p>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-3 md:gap-8 ">
              <div className="flex flex-col w-full md:w-1/2 h-[350px]">
                <p className="text-base mb-4 font-sans font-medium">
                  Proof of Insurance *
                </p>
                {/* Left: Upload Image Div */}
                <div className="flex flex-col p-5  border border-gray-300 rounded-xl">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-3 md:gap-8  ">
              <div className="flex flex-col w-full md:w-1/2 h-[350px] ">
                <p className="text-base mb-1 font-sans font-medium">
                  Proof of Ownership *
                </p>
